...
09/11/2015	               Sreeramamurthy Tripuramallu   */
...
int main(int argc, char *argv[]) {
   ...
   for (i = 0; i < 8; i = i + 2) {
    ...
    differnece1 = SallyTimeline[i+2] - SallyTimeline[i]; 
	for (j = 0; j < difference; j++) 
	   newHarryTimeline[HarryTimeline[i] - 1986 + j] = HarryTimeline[i+1]; 
    }   
    for (j = 0; j < difference1; j++) {
	   newSallyTimeline[SallyTimeline[i] - 1986 + j] = SallyTimeLine[i+1]; 
    }
	int check = 1
	for (i = 0; i < 30; i++) {
		if (newHarryTimeline[i] == newSallyTimeline[i] && newHarryTimeline[i] != -1 && check == 1) {
			Year = i + 1986; 
			check == 0; 
		}
	}
